...
import LeftPanel from '../../packages/editor/LeftPanel';
import MiddlePanel from '../../packages/editor/MiddlePanel';
import RightPanel from '../../packages/editor/RightPanel';
import { 
  projectAtom,
  updateProjectAtom,
  selectedElementAtom,
  ...
  fileStorageAtom,
  type ChatMode
} from '../../packages/editor/atoms';
import { createServerSafeProject } from '../../packages/editor/utils/projectUtils';
      ...
      <div className="flex w-full h-full overflow-hidden">
        ...
        <div className="w-80 bg-white dark:bg-boxdark border-l border-gray-200 dark:border-strokedark flex-shrink-0 overflow-hidden">
          <RightPanel
            ...
            setShowShareDialog={setShowShareDialog}
            setShowJsonModelDialog={setShowJsonModelDialog}
            onShare={async (args: { encryptedData: string; projectName: string }) => {
              try {
                return await shareProject(args);
              } catch (error) {
            ...
            onHandleMessage={async (message: string, chatMode?: ChatMode) => {
              ...
              // Process the AI prompt using the placeholder operation
              if (project) {
                try {
